...
export function GlobalStarRoom() {
  ...
  return (
    <section className="relative overflow-hidden bg-rosewash py-6 text-ink">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_72%_18%,rgba(230,0,63,.16),transparent_24%),linear-gradient(180deg,#fff0f5_0%,#fffafd_58%,#ffffff_100%)]" />
      ...
      <div className="section-shell relative">
        <div className="mb-4 flex items-end justify-between gap-3">
          <div>
            ...
            <h2 className="mt-2 text-xl font-black leading-tight sm:text-2xl">월드 팬룸</h2>
          </div>
          <span className="hidden rounded-lg bg-paper px-2 py-1 text-[10px] font-black text-cherry shadow-[0_8px_20px_rgba(230,0,63,0.08)] sm:inline-flex">
            LIVE / COS / DROP
          </span>
        </div>
        ...
        <div className="grid gap-2 lg:grid-cols-[1.12fr_0.88fr]">
          <div className="global-gloss grid gap-2 rounded-lg border border-cherry/20 bg-paper/90 p-2 shadow-lift sm:grid-cols-[1fr_0.7fr]">
            <div className="relative min-h-[320px] overflow-hidden rounded-lg bg-paper">
              ...
              <div className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-lg bg-paper/95 px-2 py-1 text-[10px] font-black text-cherry shadow-glow">
                <Star aria-hidden className="h-3.5 w-3.5 fill-current" />
                COVER PICK
              </div>
              <div className="absolute bottom-3 left-3 right-3">
                <p className="text-[10px] font-black text-paper/80">CHANMI MAGAZINE</p>
                <h3 className="mt-1 text-2xl font-black leading-none text-paper">PINK RED WHITE</h3>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2 sm:grid-cols-1">
              {visuals.slice(1, 4).map((item, index) => (
                ...
                >
                  ...
                  <p className="absolute bottom-2 left-2 rounded-lg bg-paper/95 px-1.5 py-0.5 text-[9px] font-black text-cherry">
          ...
          <div className="grid gap-2">
              ...
              <div className="rounded-lg bg-rosewash p-3">
                <div className="flex items-start gap-2">
                  ...
                  <div className="min-w-0">
                    <h3 className="text-lg font-black leading-tight text-ink">{active.title}</h3>
                ...
                {active.external ? (
                  <a
                    ...
                    className="focus-ring mt-3 inline-flex min-h-8 items-center gap-2 rounded-lg bg-cherry px-3 text-xs font-black text-paper shadow-glow"
                  >
                    바로 보기
            ...
            <div className="grid grid-cols-3 gap-2">
              {starSignals.map((signal) => {
                ...
                return (
                  <div key={signal.label} className="global-gloss rounded-lg border border-cherry/20 bg-paper p-2.5 shadow-[0_12px_28px_rgba(230,0,63,0.10)]">
                    <Icon aria-hidden className="h-4 w-4 text-cherry" />
                    <p className="mt-1 text-[9px] font-black text-cherry">{signal.label}</p>
                    <p className="mt-0.5 text-lg font-black leading-none text-ink">{signal.value}</p>
                  </div>
                );
              })}
            </div>
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="global-gloss group grid grid-cols-[88px_1fr] gap-2 rounded-lg border border-cherry/20 bg-paper p-2 shadow-[0_12px_28px_rgba(230,0,63,0.10)]"
            >
              ...
              <span className="min-w-0 self-center">
                <span className="block text-[10px] font-black text-cherry">REPLAY DROP</span>
                <span className="mt-0.5 line-clamp-2 block text-sm font-black leading-5 text-ink">{wideDrop?.title || '찬미 영상'}</span>
              </span>
            </a>
          </div>
        </div>
