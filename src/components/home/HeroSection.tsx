...
export function HeroSection() {
  ...
  return (
    ...
    >
        ...
        {heroPool.map((item, index) => (
          ...
          >
            <SmartImage
              ...
              className="h-full w-full object-cover opacity-86 saturate-100 transition duration-700 group-hover:scale-110 group-hover:opacity-100"
            />
      ...
      <div className="absolute inset-0 bg-[linear-gradient(90deg,rgba(251,248,246,.96)_0%,rgba(245,236,231,.8)_46%,rgba(251,248,246,.32)_100%)]" />
      ...
      <div className="section-shell relative flex min-h-[62svh] items-end pb-8 pt-16">
        <div className="grid w-full gap-5 lg:grid-cols-[0.64fr_1fr] lg:items-end">
          <div className="max-w-xl">
            <p className="inline-flex min-h-7 rotate-[-1deg] items-center gap-2 rounded-lg bg-cherry px-2.5 text-[11px] font-semibold text-paper shadow-glow">
              <Sparkles aria-hidden className="h-3.5 w-3.5 text-paper" />
              LIVE / NYAM / COS
            </p>
            <h1 className="mt-3 pb-1 text-3xl font-semibold leading-tight text-ink sm:text-4xl">나는 찬미</h1>
          ...
          >
            <div className="relative aspect-video overflow-hidden rounded-lg bg-rosewash">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-3">
                <p className="text-[10px] font-semibold text-paper">NOW CUT</p>
                <h2 className="mt-1 line-clamp-1 text-lg font-semibold">{current.title}</h2>
              </div>
            </div>
          </a>
        </div>
      ...
      <div className="marquee absolute bottom-0 left-0 right-0 border-y border-cherry/20 bg-paper/75 py-1.5 text-[11px] font-semibold text-cherry backdrop-blur">
        <div>
          {heroPool.map((item) => (
